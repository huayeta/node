...
;!function(win){
    ...
    var sCss = '';
    sCss += '.sim-pop-box{position: absolute; left: 0; top: 0; width: 100%; z-index: 9999;color: #000;font-size: 12px;font-family: Helvetica, STHeiti, Droid Sans Fallback;}';
...
//    sCss += '.sim-pop-box *{margin: 0; padding: 0}';
    ...
    sCss += '.sim-pop-com,.sim-pop-loading{min-width: 200px; max-width: 95%; background-color: #fff;border: 1px solid #999;border-radius: 5px;box-shadow: 0 0 5px rgba(0, 0, 0, 0.2); }';
    ...
    Simpop.prototype = {
        ...
        mainCon: function(){
            ...
            //content
            temp += '<div class="sim-pop-bd" style="'+this.opt.style+'">'+ this.opt.content +'</div>';
            //btn
        ...
        handler: function(){
            ...
            if(self.ok.callback){
                ele.querySelector('.btn-sure') && ele.querySelector('.btn-sure').addEventListener('click',function(){
                    if(self.opt.autoClose)self.close();
